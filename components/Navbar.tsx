  ...
  return (
    <motion.nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ...
          : 'bg-navy/30 backdrop-blur-sm'   // ← subtle dark tint over hero so logo + links are always visible
      }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20"> {/* h-20 gives logo more breathing room */}
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <motion.button
              ...
              className="bg-amber text-navy px-6 py-2 rounded-lg font-semibold text-sm hover:bg-amber/90 transition-colors duration-200"
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="px-4 py-4 space-y-4">
              ...
              <motion.button
                onClick={() => scrollToSection('#contact')}
                className="w-full bg-amber text-navy px-6 py-2 rounded-lg font-semibold text-sm hover:bg-amber/90 transition-colors duration-200"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                Get Started
